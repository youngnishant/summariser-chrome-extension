  ...
  waitForElm("#secondary.style-scope.ytd-watch-flexy").then(() => {
      ...
      .insertAdjacentHTML(
        ...
        <div class="yt_ai_summary_container">
            <div id="yt_ai_summary_header" class="yt_ai_summary_header">
                ...
                <p class="yt_ai_summary_header_text">Youtuber Summarizer</p>
            ...
            <div id="yt_ai_summary_body" class="yt_ai_summary_body">
                ...
                <div id="yt_ai_summarised_text" class="yt_ai_summary_text"></div>
    ...
    document
      ...
      .addEventListener("click", async (e) => {
        ...
        document.querySelector("#yt_ai_summarised_text").innerHTML =
          "Summary: Loading...";
        ...
        const response = await fetch(
          `https://summarise.vercel.app/api/summarise?type=normal&id=${videoId}`
        ).then((res) => res.json());
        const summarisedText = response.summary[0].trim();
        console.log("xxx", summarisedText);
        document.querySelector("#yt_ai_summarised_text").innerHTML =
...
function sanitizeWidget() {
  ...
  // Height Adjust
  document.querySelector("#yt_ai_summary_body").style.maxHeight =
...
function createLangSelectBtns(langOptionsWithLink) {
  document.querySelector("#yt_ai_summary_lang_select").innerHTML = Array.from(
    langOptionsWithLink
  )
    .map((langOption, index) => {
      return `<button class="yt_ai_summary_lang ${
        index == 0 ? "yt_ai_summary_lange_selected" : ""
      }" data-yt-transcript-lang="${langOption.language}">${
        langOption.language
      }</button>`;
    })
    .join("");
...
function evtListenerOnLangBtns(langOptionsWithLink, videoId) {
  Array.from(document.getElementsByClassName("yt_ai_summary_lang")).forEach(
    (langBtn) => {
      langBtn.addEventListener("click", async (e) => {
        const lang = e.target.getAttribute("data-yt-transcript-lang");
        const targetBtn = document.querySelector(
          `.yt_ai_summary_lang[data-yt-transcript-lang="${lang}"]`
        );
        const link = langOptionsWithLink.find(
          (langOption) => langOption.language === lang
        ).link;
...
function scrollIntoCurrTimeDiv() {
  ...
  Array.from(
    document.getElementsByClassName("yt_ai_summary_transcript_text_timestamp")
  ).forEach((el, i, arr) => {
    const startTimeOfEl = el.getAttribute("data-start-time");
    const startTimeOfNextEl =
      i === arr.length - 1
        ? getTYEndTime()
        : arr[i + 1].getAttribute("data-start-time") ?? 0;
    if (currTime >= startTimeOfEl && currTime < startTimeOfNextEl) {
      el.scrollIntoView({ behavior: "auto", block: "start" });
      document
        .querySelector("#secondary > div.yt_ai_summary_container")
...
function evtListenerOnTimestamp() {
  Array.from(
    ...
    el.addEventListener("click", (e) => {
      ...
      const ytVideoEl = document.querySelector(
...
function copyTranscript(videoId) {
  ...
  Array.from(document.getElementById("yt_ai_summary_text").children).forEach(
    (el) => {
      if (!el) {
        return;
      }
      ...
      const timestampHref = el
        .querySelector(".yt_ai_summary_transcript_text_timestamp")
...
function copyTranscriptAndPrompt() {
  const textEls = document.getElementsByClassName(
